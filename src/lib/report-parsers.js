...
function isTotalLikeLabel(value) {
  return String(value || "").toLowerCase().includes("total");
}
  ...
  rows.forEach(row => {
    ...
    if (row.ColData && Array.isArray(row.ColData)) {
      ...
      if (!isTotalLikeLabel(label) && columns.length >= 2) {
        ...
        transactions.push({
          id: String(columns[0]?.id || row.id || Math.random().toString(36).slice(2, 9)),
          date: label || reportDate,
          type: columns[1]?.value || "Transaction",
          num: columns[2]?.value || "",
          name: columns[3]?.value || columns[4]?.value || columns[5]?.value || "",
    ...
    if (subRows.length > 0) {
      transactions.push(...extractTransactions(subRows, reportDate, columnsMetadata));
    }
  });
  ...
  for (const row of rows) {
    ...
    if (rowData.length > 0) {
      const directData = rowData.filter(
        (child) => String(child?.type || "").toLowerCase() === "data" || Array.isArray(child?.ColData),
      );

      if (directData.length > 0) {
        accounts.push({
          id: String(row?.id || `acc-${Math.random().toString(36).slice(2, 7)}`),
          name: String(headerName || "").replace(/^Total\s+/i, ""),
          total,
          transactions: extractTransactions(directData, reportDate, columnsMetadata),
        });
      }
  ...
  for (const row of rows) {
    ...
    if (directData.length > 0) {
      groups.push({
        ...
        accounts: [
          {
            id: String(row?.Header?.ColData?.[0]?.id || row?.id || `acc-${normalizeKey(groupName)}`),
            name: groupName,
            total,
            transactions: extractTransactions(directData, reportDate, columnsMetadata),
          },
    ...
    if (Array.isArray(row?.Rows?.Row)) {
      const subData = parseDetailRows(row.Rows.Row, reportDate, columnsMetadata);
...
export function parseDetailReport(payload) {
  const columnsMetadata = payload?.Columns?.Column || payload?.data?.Columns?.Column || [];
...
function balanceSheetCollectAccounts(row, reportDate, ledgerIndex) {
  ...
  childRows.forEach((child, index) => {
    ...
    if (type === "data" || Array.isArray(child?.ColData)) {
      ...
      const accountId = child?.ColData?.[0]?.id ? String(child.ColData[0].id) : undefined;
    ...
    if (type === "section" || isSectionLikeRow(child)) {
      collected.push(...balanceSheetCollectAccounts(child, reportDate, ledgerIndex));
    }
  });
...
export function parseBalanceSheetDetailFromAllReports(payload, fallbackDate = "N/A") {
  try {
    ...
    const reportDate = getReportDate(generalLedgerReport || balanceSheetReport, fallbackDate) || fallbackDate;
    ...
    const extractDeDuped = (ledgerNode) => {
      ...
      return raw.filter(tx => {
        ...
        const key = `${tx.date}-${tx.type}-${tx.num}-${tx.amount}-${tx.balance}-${tx.memo}-${tx.name}`.toLowerCase();
    ...
    const traverseBS = (rows, parentName = "General") => {
      ...
      list.forEach(row => {
        ...
        if (isDataRow || transactions.length > 0) {
          const finalTransactions = transactions.length > 0 ? transactions : [
            {
...
function cashflowAddTransaction(transactionMap, accountIds, accountNames, transaction) {
  const keys = new Set();
  ...
  asArray(transactions?.invoices?.Invoice).forEach((invoice, index) => {
    ...
    cashflowAddTransaction(
      ...
      ["Accounts Receivable (A/R)"],
      {
        ...
        memo: invoice?.PrivateNote || cashflowLineDescription(invoice?.Line?.[0]),
  ...
  asArray(transactions?.payments?.Payment).forEach((payment, index) => {
    ...
    cashflowAddTransaction(
      ...
      ["Accounts Receivable (A/R)"],
      {
        ...
        memo: payment?.PrivateNote || cashflowLineDescription(payment?.Line?.[0]),
  ...
  asArray(transactions?.purchases?.Purchase).forEach((purchase, index) => {
    ...
    cashflowAddTransaction(
      ...
      [accountName],
      {
        ...
        memo: purchase?.PrivateNote || cashflowLineDescription(purchase?.Line?.[0]),
        split: accountName || "Expense Account",
        amount,
        balance: amount,
      },
...
function cashflowGetAccountTransactions(accountId, accountName, total, reportDate, transactionMap) {
  const byId = accountId ? transactionMap.get(`id:${accountId}`) : undefined;
  const byName = transactionMap.get(`name:${cashflowNormalizeKey(accountName)}`);
  const transactions = [...(byId || []), ...(byName || [])];

  if (transactions.length > 0) {
    return transactions.filter(
      (transaction, index, self) =>
        index === self.findIndex((candidate) => candidate.id === transaction.id),
    );
  }
  ...
  asArray(rows).forEach((row, index) => {
    ...
    if (type === "section") {
      ...
      if (nestedRows.length > 0) {
        accounts.push(...cashflowCollectAccounts(nestedRows, reportDate, transactionMap));
        return;
      }
...
export function parseCashflowEngineDetailReport(payload, fallbackDate) {
  ...
  const groups = asArray(rows).map((row, index) => {
    ...
    const accounts = row?.Rows?.Row
      ? cashflowCollectAccounts(asArray(row.Rows.Row), reportDate, transactionMap)
      : [
        {
          id: String(
            row?.group ||
    ...
    return {
      id: String(
        row?.group || cashflowCreateStableId("cashflow-group", groupName, index),
      ),
      name: groupName,
      total,
      accounts,
  ...
  return {
    groups,
    grandTotal: groups.reduce((sum, group) => sum + Number(group.total || 0), 0),
  };
}
