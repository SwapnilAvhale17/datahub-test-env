...
function buildFrontendHashUrl(baseUrl, hashPath, searchParams = "") {
  const normalizedHash = hashPath?.startsWith("/") ? hashPath : "/broker/companies";
...
router.get("/refresh-token", async (req, res) => {
  ...
  try {
    ...
    console.error(`❌ Token refresh failed for client ${clientId}:`, error.response?.data || error.message);
...
router.get("/api/auth/quickbooks", (req, res) => {
  const clientId = getClientId(req);
  if (!clientId) {
    return res.status(400).json({ error: "Cannot start OAuth without Client ID" });
  }

  const qb = getQBConfig(clientId);
  const qbClientId = qb.clientId || process.env.QB_CLIENT_ID;
  const appBaseUrl = getAppBaseUrl(req);
  const redirectUri = process.env.QB_REDIRECT_URI || `${appBaseUrl}/api/auth/callback`;
  const scope = "com.intuit.quickbooks.accounting";
  
  // The state carries both the redirect path and our internal clientId
  const state = req.query.state || encodeURIComponent(JSON.stringify({ redirect: "/broker/companies", clientId }));
...
router.get("/api/auth/callback", async (req, res) => {
  ...
  if (!code || !realmId || !clientId) {
    ...
    return res.redirect(buildFrontendHashUrl(frontendUrl, redirectHash, "?qbStatus=error&qbMessage=Invalid+callback+data"));
  }

  const qb = getQBConfig(clientId);
  const qbClientId = qb.clientId || process.env.QB_CLIENT_ID;
  const qbClientSecret = qb.clientSecret || process.env.QB_CLIENT_SECRET;
  const basicToken = Buffer.from(`${qbClientId}:${qbClientSecret}`).toString("base64");
  const redirectUri = process.env.QB_REDIRECT_URI || `${appBaseUrl}/api/auth/callback`;
  ...
  try {
    ...
    const tokenExpiresAt = new Date(Date.now() + (tokenResponse.data.expires_in || 3600) * 1000).toISOString();
    ...
    const tokenData = {
      ...
      syncedEntities: ["Customers", "Invoices", "Balance Sheet", "General Ledger", "Profit and Loss"],
    };
    ...
    try {
      ...
      if (info) {
        ...
        console.log(`🏢 Connected to Company: ${info.CompanyName} for Client: ${clientId}`);
      }
    } catch (companyErr) {
    ...
    return res.redirect(buildFrontendHashUrl(frontendUrl, redirectHash, "?qbStatus=success"));
  } catch (error) {
    console.error("❌ QuickBooks Callback Error:", error.response?.data || error.message);
    return res.redirect(buildFrontendHashUrl(frontendUrl, redirectHash, "?qbStatus=error&qbMessage=OAuth+exchange+failed"));
  }
});
...
router.get("/api/auth/status", (req, res) => {
  const clientId = getClientId(req);
  if (!clientId) {
    return res.json({ success: true, isConnected: false, message: "No Client ID provided" });
  }
